...
export const getForkedETH = async (amount: string, to: string) => {
    ...
    await hre.network.provider.send("hardhat_setBalance", [
        to,
        ethers.toBeHex(ethers.parseEther(amount.toString()) + balance).replace("0x0", "0x"),
    ]);
};

export const getImpersonatedSigner = async (user: string): Promise<HardhatEthersSigner> => {
...
export const getTransparentUpgradeableProxyInfo = async (
    ...
    const proxyAdminAddress = `0x${(
        await provider.getStorage(proxy, "0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103")
    ).slice(-40)}`;
